...
const platOf     = e => e.plat || ((e.cat || []).includes('webapi') ? 'both' : 'win');
const platLabel  = { win: 'Windows', mac: 'macOS', both: 'Windows & macOS' };
...
idx += '> Community reference for error codes in the Spotify desktop client (Windows and macOS). '
...
idx += '## Error codes\n';
for (const g of groups) for (const e of g.items) idx += `- ${e.code} - ${stripTags(e.title)} (${e.sev}, ${platLabel[platOf(e)]})\n`;

let full = '';
full += '# Spotify Desktop Error Reference - Full Reference\n\n';
full += `Source: ${SITE}\nRepository: ${REPO}\n`;
full += `Scope: Windows and macOS desktop client. ${E.length} error codes documented.\n`;
full += 'Most client errors are shared across Windows and macOS; the installer codes differ per OS. '
      + 'Each entry below is tagged with the platform it applies to.\n';
...
for (const g of groups) {
  ...
  for (const e of g.items) {
    ...
    full += `Severity: ${e.sev}\n`;
    full += `Platform: ${platLabel[platOf(e)]}\n`;
